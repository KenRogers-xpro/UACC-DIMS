...
export function useMessages() {
  ...
  const hideConversation = useCallback(async (otherUserId) => {
    const res = await api.delete(`/messages/conversations/${otherUserId}`)
    if (!res.success) throw new Error(res.message || 'Failed to hide conversation')
    await fetchConversations()
  }, [fetchConversations])

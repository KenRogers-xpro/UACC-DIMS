...
export function useCirculation() {
  ...
  const fetchInbox = useCallback(async () => {
    ...
    try {
      ...
      const data = res.circulations || res.data?.circulations || res.data || []
  ...
  const fetchTimeline = useCallback(async (id) => {
    ...
    try {
      ...
      const data = res.circulation || res.data?.circulation || res.data || null
      setTimeline(data)
      return data
    } catch (err) {
  ...
  const initiateCirculation = useCallback(async (data) => {
    ...
    try {
      ...
      return res.circulation || res.data?.circulation || res.data
    } catch (err) {
  ...
  const addStep = useCallback(async (id, data) => {
    ...
    try {
      ...
      return res.step || res.data?.step || res.data
    } catch (err) {
  ...
  // Callable by whoever is currentHolderRole right now, at any point in the
  // circulation's life (not just once CLOSED) — enforced server-side
  // (circulation.routes.js POST /:id/send-to-file), not just hidden in the
  // UI. Can legitimately be called more than once over a document's life,
  // by different holders at different times.
